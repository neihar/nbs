    ...
    Session->CreateHandle(callContext, std::move(request))
        .Subscribe([=, ptr = weak_from_this()] (const auto& future) {
            ...
            if (auto self = ptr.lock(); CheckResponse(self, *callContext, req, response)) {
                ...
                if (self->Config->GetGuestCacheDisabled()) {
                    fi.direct_io = 1;
                }
    ...
    fuse_file_info* fi)
{
    ...
    for (int index = 1; index < count; index++) {
        ...
        request->Sglist.emplace_back(
            static_cast<char*>(iov[index].iov_base),
            std::min(remainingSize, iov[index].iov_len));
    ...
    fuse_file_info* fi)
{
    ...
    request->SetOffset(offset);
    request->Length = size;
    request->Sglist.reserve(bufv->count);
